...
public class WishlistRepository {
	...
	private List<WishList> generateWishList() {
		...
		wishLists.add(WishList.builder()
				...
				.title("Так не хочется ни куда вставать.")
				.description("Утро, а так не хочется ни чего делать... Но у тебя есть выход, активируешь желание и муж все сделает за тебя. Оденет и отведет детей в садик. А после этого приедет за тобой")
				.count(5)
				.build());
		...
		wishLists.add(WishList.builder()
				...
				.description("Ну что ж теперь не отвертишься. Собираемся и едим по магазинам в поисках чего-нибудь интересного. Необходимо предупредить за 2 недели.")
				.count(1)
				.price(6000f)
				.build());
		...
		wishLists.add(WishList.builder()
				...
				.description("Так хочется чего-нибудь вкусненького на завтрак. И при этом не хочется вставать...")
				.count(7)
				.build());
